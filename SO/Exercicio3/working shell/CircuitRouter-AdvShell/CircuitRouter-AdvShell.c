...
void printChildren(vector_t *children) {
    for (int i = 0; i < vector_getSize(children); ++i) {
        ...
        if (pid != -1) {
            ...
            int exec_time = TIMER_DIFF_SECONDS(child->start, child->stop);
...
void invalidSyntax(char buffer[], char *args[])
{
    ...
    if((clientFD = open(clientPipe, O_NONBLOCK | O_WRONLY)) == -1){
...
int main (int argc, char** argv) {
    ...
    while (1) {
        ...
        if((select(fdFIFO_IN+1, &fdset, NULL, NULL, NULL)) == -1) {
            if(errno == EINTR)
                continue;
            else
            {
        ...
        if(FD_ISSET(STDIN_FILENO,&fdset)){
            ...
            if((readaux = read(fdFIFO_IN, buffer, BUFFER_SIZE)) == -1){
